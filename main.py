...
class SpatialAttention(nn.Module):
    def __init__(self, dim, heads = 12, dim_head = 8):
        super().__init__()
...
class BottleneckBlock(nn.Module):
    ...
    def forward(self, x):
        residual = x
        atten_x = self.drop(self.attention(x))
        atten_x += residual
...
class MultiModalAttention(nn.Module):
    def __init__(self, img_dim, q_dim, hidden_dim, heads=24):
        super().__init__()
        self.heads = heads
    ...
    def forward(self, features1, features2):
        ...
        return self.norm(attended_features + res) #[batch, hidden_dim*2]
...
class VQAModel(nn.Module):
    def __init__(self, n_answer: int):
        ...
        self.multi_attention = MultiModalAttention(img_dim=512, q_dim=312, hidden_dim=48, heads=24)
        self.self_attention = SelfAttention(hidden_dim=48*2, heads=24)
        self.norm = nn.LayerNorm(48*2)
        self.drop = nn.Dropout(0.1)
        self.fc = nn.Sequential(
            nn.Linear(48*2, 48*8),
            nn.ReLU(inplace=True),
            nn.Linear(48*8, 48*16),
            nn.ReLU(inplace=True),
            nn.Linear(48*16, n_answer)
        )
